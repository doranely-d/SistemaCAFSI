...
<h2 class="text-center">Asistencia de Pacientes</h2><br>

<script type="text/javascript">
    function buscarPorID() {
        var id = document.getElementById("id").value;
        if (id.toString() !== "") {
            location.href = "citas_paciente.php?id=" + id;
        } else {
...
<form action="asistencia_paciente.php" method="post">
...
if (isset($_POST["submit"])) {
    ...
    if (empty($nombre) && empty($apellido)) {
        Errors::mostrar_error("asistencia_paciente.php", 6);
                    ...
                    <table border="1" class="tabla2" >
                        ...
                        while ($registro = $resultado->fetch_array()) {
                            ...
                            <tr style="background-color: <?php echo($hipertension == 1? "orange" : "white") ?>">
                                ...
                                <td><input type="button" value="Elegir..." class="btn btn-primary" onclick="location.href = 'citas_paciente.php?id=<?php echo($bd_id_paciente); ?>'"></td>
                            </tr>
                            <?php
                        }
                        $bd->close();
